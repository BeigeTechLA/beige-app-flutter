...
class BookingMoreDetailsScreen extends StatefulWidget {
  final int bookingId;
  final String contentType;
  const BookingMoreDetailsScreen({super.key, required this.bookingId, required this.contentType});

  @override
  State<BookingMoreDetailsScreen> createState() => _BookingMoreDetailsScreenState();
}

class _BookingMoreDetailsScreenState extends State<BookingMoreDetailsScreen> {
...
/*  Navigator.push(
  ...
  Map<String, int> _buildCrewRequirements() {
    ...
    final typeId = getContentTypeId();

    if ((typeId == 1 || typeId == 3) && additionalVideoQty > 0) {
      crew["videographer"] = additionalVideoQty;
    }

    if ((typeId == 2 || typeId == 3) && additionalPhotoQty > 0) {
  ...
  String getTopSummaryText() {
    ...
    if (typeId == 1 || typeId == 3) {
      final total = includedVideoQty + additionalVideoQty;
      if (total > 0) {
        parts.add("Videography x $total");
      }
    }

    if (typeId == 2 || typeId == 3) {
      final total = includedPhotoQty + additionalPhotoQty;
      if (total > 0) {
        parts.add("Photography x $total");
      }
    }
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (getContentTypeId() == 2 || getContentTypeId() == 3)
