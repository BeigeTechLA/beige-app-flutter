...
class CrewSizeMatchingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CrewSizeMatchingScreen> createState() =>
      _CrewSizeMatchingScreenState();
}

class _CrewSizeMatchingScreenState
    extends ConsumerState<CrewSizeMatchingScreen> {
  int currentStep = 1;
  ...
  Widget build(BuildContext context) {
    final crewState = ref.watch(
      crewRecommendationNotifierProvider(widget.bookingId),
    );
    final crewData = crewState.data;
    // Extract data from notifier state
    final shootName =
        (crewData['shoot_type'] as Map?)?['name'] as String? ?? "";
    final contentType =
        (crewData['shoot_type'] as Map?)?['content_type'] as String? ?? "";
    final shootImageUrl =
        (crewData['shoot_type'] as Map?)?['image_url'] as String? ?? "";
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                Text("Crew Size & Matching", style: AppTextStyles.titleSmall),
              ],
            ),
