...
void main() {
  ...
  test('addParticipant trims + dedupes', () async {
    final repo = _FakeRepo();
    final container = await _container(repo: repo);
    addTearDown(container.dispose);
    ...
    final n = container.read(createMeetingNotifierProvider.notifier);
    n.addParticipant('  Alice  ');
    n.addParticipant('Alice'); // duplicate
    n.addParticipant('   '); // empty

    expect(
      container.read(createMeetingNotifierProvider).invitedParticipants,
      ['Alice'],
    );
  });
}
